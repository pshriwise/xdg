...
xdg::Vertex LibMeshManager::vertex_coordinates(MeshID vertex_id) const {
...
std::vector<MeshID> LibMeshManager::element_connectivity(MeshID element) const {
  const auto elem_ptr = mesh()->elem_ptr(element);
  if (!elem_ptr) {
    fatal_error("Invalid element ID in element_connectivity");
  }
